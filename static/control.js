...
function bindActions() {
  ...
  document.querySelectorAll(".position-price-change").forEach(button=>button.addEventListener("click",async()=>{
    const suggested=button.dataset.liquidation||button.dataset.current;
    const entered=prompt(`CHANGE ${button.dataset.ticker} MARKET PRICE\n\nThis is the shared FCX quote and will immediately affect every account holding or trading this security. The selected position liquidation price is ${money(suggested)}.\n\nEnter the new market price:`,suggested);
    if(entered===null)return;
    const target=Number(entered);
    if(!Number.isFinite(target)||target<=0)return notice("Enter a valid market price greater than zero.",true);
    const reason=prompt("Required FEC reason for this market intervention (minimum 10 characters):");
    if(reason===null)return;
    if(reason.trim().length<10)return notice("Document a reason of at least 10 characters.",true);
    if(!confirm(`CONFIRM IMMEDIATE PRICE CHANGE?\n\n${button.dataset.ticker} will move from ${money(button.dataset.current)} to ${money(target)} for the entire FCX market. This may liquidate one or more leveraged positions and will be permanently audit logged.`))return;
    await mutation(`/admin/live-positions/${button.dataset.id}/price`,"POST",{target_price:target,reason:reason.trim()},`${button.dataset.ticker} market price changed and recorded in the FEC audit ledger.`,state.section);
  }));
