...
const sections = [
  ...
  ["accounts", "Ravenhood Accounts"], ["banking", "FCX Banking"], ["investigations", "FEC Investigations"], ["seized", "Seized Assets"], ["audit", "Audit / Activity"],
...
const sectionMeta = {
  ...
  accounts: ["Regulatory", "◎"], banking: ["Regulatory", "$"], investigations: ["Regulatory", "⌕"], seized: ["Regulatory", "▣"], audit: ["Regulatory", "≡"],
...
function visibleSections() {
  ...
  if (roles.has("fec_investigator")) return sections.filter(([key]) => ["overview", "positions", "accounts", "banking", "investigations", "seized", "health", "audit"].includes(key));
...
const apiError = (detail, fallback) => {
  ...
  if (Array.isArray(detail)) return detail.map(item => {
    ...
    return `${field || "request"}: ${item?.msg || safeJson(item)}`;
  }).join("; ");
  if (detail && typeof detail === "object") return detail.message || detail.msg || safeJson(detail);
  return fallback;
};
...
async function request(path, options = {}) {
  ...
  if (!response.ok) throw new Error(apiError(payload.detail, `Request failed (${response.status})`));
  return payload;
}
...
async function openSection(section, force = false) {
  ...
  $("#section-kicker").textContent = ["investigations", "seized", "promotions"].includes(section) ? "FEDERAL EXCHANGE COMMISSION" : "RAVENHOOD EXCHANGE OPERATIONS";
...
function sectionHead(kicker, title, description = "") {
  const enforcement=kicker==="FEC INVESTIGATIONS"?activeInvestigationRestrictions():"";
  return `<div class="section-head"><div><p class="eyebrow">${esc(kicker)}</p><h3>${esc(title)}</h3><p>${esc(description)}</p></div></div>${enforcement}`;
}
...
async function renderOperations() {
  ...
  const securities = data.securities || [], tradable = securities.filter(row => row.exchange_status === "active"), listingCandidates = securities.filter(row => row.exchange_status !== "delisted");
  ...
  return `${sectionHead("FCX OPERATIONS", "Scheduled movement and listing authority", "Move one security, a selected group, or an index; preserve final prices; halt, delist, resume, and relist from one desk.")}
    ${metrics([["Active securities", number(securities.filter(row => row.exchange_status === "active").length), `${number(securities.length)} registered`],["Active programs", number(activePrograms.length), `${number(data.programs?.length)} retained in history`],["Active halts", number(securities.filter(row => row.exchange_status === "halted").length), "FEC restrictions live"],["Delisted", number(securities.filter(row => row.exchange_status === "delisted").length), "Preserved off-exchange records"],["Index funds", number(data.funds?.length), "FCXS / FCXV state"],["Price authority", "Online", "Immediate or future programs"]])}
    ${sectionHead("EXCHANGE DIRECTORY", "Active, halted, and delisted securities", "Every registered security remains visible after a halt or delisting. Search or filter the authoritative exchange state below.")}
    <article class="panel wide"><div class="account-directory-controls"><label><span>Search</span><input id="security-directory-search" type="search" placeholder="Ticker, company, reason, or case"></label><label><span>Exchange status</span><select id="security-directory-status"><option value="">All securities</option><option value="active">Active</option><option value="halted">Halted</option><option value="delisted">Delisted</option><option value="inactive">Inactive</option></select></label><strong id="security-directory-count">${number(securities.length)} shown</strong></div><div id="security-directory-table">${dataTable(["Ticker","Security","Price","Type","Exchange status","Reason / notice","Case","Effective","Action"],securities,row=>{const status=String(row.exchange_status||"inactive"),reason=status==="halted"?(row.halt_reason_label||row.halt_reason_code):status==="delisted"?(row.delisting_reason_label||row.delisting_reason_code):"Trading normally",notice=status==="halted"?row.halt_public_notice:status==="delisted"?row.delisting_public_notice:"",caseRef=status==="halted"?row.halt_case_reference:status==="delisted"?row.delisting_case_reference:"",effective=status==="halted"?row.halted_at:status==="delisted"?row.delisted_at:row.updated_at,action=status==="halted"?`<button class="resume-halt" data-id="${row.active_halt_id}">Resume</button>`:status==="delisted"?`<button class="relist-security" data-id="${row.active_delisting_id}">Relist on FCX</button>`:"—",search=`${row.ticker} ${row.name} ${reason} ${notice} ${caseRef}`.toLowerCase();return `<tr data-security-directory-row data-status="${esc(status)}" data-search="${esc(search)}"><td><strong>${esc(row.ticker)}</strong></td><td>${esc(row.name)}</td><td>${money(row.price)}</td><td>${esc(row.security_type)}</td><td><span class="status-chip">${esc(status.toUpperCase())}</span></td><td>${esc(reason)}${notice?`<small>${esc(notice)}</small>`:""}</td><td>${esc(caseRef||"—")}</td><td>${esc(stamp(effective))}</td><td>${action}</td></tr>`;})}</div><div class="empty compact" id="security-directory-empty" hidden>No securities match this filter.</div></article>
    <div class="grid operator-grid">
      <article class="panel"><h4>Schedule movement</h4><p>Select multiple securities. The current price and calculated target remain visible before execution.</p><form id="price-program-form" class="form-grid"><label class="full">Securities<select name="security_ids" multiple size="10" required>${selectOptions(tradable)}</select></label><label class="full">RP event<input name="event_name" placeholder="Documented market event" required></label><label>Percentage change<input name="percent_change" type="number" step="0.01" min="-99.99" max="1000" required></label><label>Duration (minutes)<input name="duration_minutes" type="number" min="1" max="43200" value="60" required></label><label class="full">Start time (blank = now)<input name="starts_at" type="datetime-local"></label><div id="program-preview" class="price-preview full">Select securities to preview current and target prices.</div><button class="primary full">Launch price program</button></form></article>
      <article class="panel"><h4>FEC market intervention</h4><form id="halt-form" class="form-grid"><label class="full">Securities<select name="security_ids" multiple size="6" required>${selectOptions(tradable)}</select></label><label>Reason code<input name="reason_code" value="FEC_REVIEW" required></label><label>Reason label<input name="reason_label" value="FEC market review" required></label><label class="full">Public notice<textarea name="public_notice" required></textarea></label><label>Case reference<input name="case_reference"></label><label>Automatic resume<input name="automatic_resume_at" type="datetime-local"></label><button class="danger-action full">Halt selected securities</button></form><div class="panel-actions"><button id="resume-all-halts">Resume all active halts</button></div></article>
      <article class="panel"><h4>Delist security</h4><form id="delist-form" class="form-grid"><label class="full">Security<select name="security_id">${selectOptions(listingCandidates)}</select></label><label>Reason code<input name="reason_code" value="FEC_LISTING_ACTION" required></label><label>Reason label<input name="reason_label" value="FEC listing action" required></label><label class="full">Public notice<textarea name="public_notice" required></textarea></label><label class="full">Case reference<input name="case_reference"></label><button class="danger-action full">Halt and delist</button></form></article>
...
async function renderCommandOverview() {
  ...
  const communityLeverage = yes(settings.market_margin_enabled ?? "1");
  ...
  const emergency = canAdmin ? `<div class="emergency-grid">
    ...
    ${controlTile("Community Longs & Shorts",communityLeverage?"Released":"Restricted","Resident leveraged positions across connected CADs",`<button class="market-state ${communityLeverage?"danger-action":""}" data-field="margin_enabled" data-value="${communityLeverage?"false":"true"}">${communityLeverage?"Restrict":"Release"}</button>`)}
  ...
  const alerts = (data.recent_alerts||[]).slice(0,5).map(row=>`<button class="activity-item risk-alert-review" data-id="${row.id}"><i class="down"></i><span><strong>${esc(row.title||row.alert_type||"Risk alert")}</strong><small>${esc(row.severity||"Open")} · ${esc(row.ticker||row.target||"Market")} · Review and adjust</small></span><time>${esc(stamp(row.created_at))}</time></button>`).join("")||`<div class="empty compact">No active market alerts</div>`;
  ...
  return `<div class="command-header"><div><p class="eyebrow">FCX COMMAND CENTER</p><h3>Operational overview</h3><p>Live market authority, resident exposure, community connectivity, risk signals, and FEC activity from the central FCX control plane.</p></div><div class="command-time"><strong>${esc(new Date().toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"}))}</strong><small>Last synchronized now</small></div></div>
    ...
    <div class="overview-layout"><div class="overview-main">
      ...
      <article class="panel"><div class="panel-heading"><div><h4>Live Positions Monitor</h4><p>Open resident stock holdings and leveraged exposure.</p></div><button class="open-section" data-section="positions">View all positions</button></div>${positionsTable(recentPositions)}</article>
      <div class="grid"><article class="panel"><div class="panel-heading"><div><h4>Recent FEC activity</h4><p>Latest privileged actions from the central audit ledger.</p></div><button class="open-section" data-section="audit">Open audit</button></div><div class="activity-list">${activity}</div></article><article class="panel"><div class="panel-heading"><div><h4>Recent alerts</h4><p>Click a signal to inspect its evidence and apply an advised threshold.</p></div><span class="status-chip ${(data.recent_alerts||[]).length?"warn":""}">${(data.recent_alerts||[]).length} open</span></div><div class="activity-list">${alerts}</div></article></div><div id="alert-review-target"></div>
...
function positionsTable(rows) { const canIntervene=isDeveloper()||currentRoles().has("fec_admin"); return dataTable(["Account / community","Symbol","Position","Quantity","Entry / current","Value","P/L","Leverage","Liquidation","Age",...(canIntervene?["FEC action"]:[])], rows, row=>`<tr><td><strong>${esc(row.display_name||row.account_id)}</strong><small>${esc(row.communities||"Unlinked")}</small></td><td><strong>${esc(row.ticker)}</strong><small>${esc(row.name||"")}</small></td><td><strong>${esc(String(row.position_type||"leverage").toUpperCase())}</strong><small>${esc(String(row.side||row.direction||"").toUpperCase())}</small></td><td>${number(row.quantity)}</td><td>${money(row.entry_price)}<small>${money(row.current_price||row.mark_price)}</small></td><td>${money(row.current_value||Number(row.quantity||0)*Number(row.current_price||row.mark_price||0))}</td><td>${pnl(row.unrealized_pnl)}</td><td>${row.position_type==="equity"?"1x":`${number(row.leverage)}x`}</td><td>${row.liquidation_price==null?"—":money(row.liquidation_price)}</td><td>${row.age_seconds!=null?`${number(Number(row.age_seconds||0)/3600)}h`:esc(stamp(row.opened_at))}</td>${canIntervene?`<td>${row.position_type==="leverage"?`<div class="button-row"><button class="position-price-change" data-id="${row.id}" data-ticker="${esc(row.ticker)}" data-current="${row.current_price||row.mark_price}" data-entry="${row.entry_price}" title="Set the shared quote to this position's entry price">Change price</button><button class="position-liquidate danger-action" data-id="${row.id}" data-ticker="${esc(row.ticker)}" data-account="${esc(row.display_name||row.account_id)}" title="Close this leveraged position at the current quote">Liquidate</button></div>`:"—"}</td>`:""}</tr>`, "No open stock or leverage positions"); }
function residentOpenPositions(data,account) { const identity={display_name:account?.display_name,account_id:account?.account_id,communities:account?.communities}; return [...(data.holdings||data.active_holdings||[]),...(data.active_leverage||[])].map(position=>({...position,...identity,position_type:position.position_type||(position.average_cost!=null?"equity":"leverage"),side:position.side||position.direction||(position.average_cost!=null?"long":""),entry_price:position.entry_price??position.average_cost,current_price:position.current_price??position.price,current_value:position.current_value??Number(position.quantity||0)*Number(position.current_price||position.price||0)})); }
function pendingOrdersTable(rows) { return `${sectionHead("UPCOMING STOCK ORDERS","FCX request queue",`${number(rows.length)} centrally recorded request(s) awaiting execution or settlement`)}${dataTable(["Submitted","Community","Symbol","Side","Quantity","Reference price","Reserved value","Status"],rows,row=>`<tr><td>${esc(stamp(row.created_at))}</td><td>${esc(row.community_id)}</td><td><strong>${esc(row.ticker)}</strong></td><td>${esc(String(row.side||"").toUpperCase())}</td><td>${number(row.quantity)}</td><td>${money(row.submitted_price)}</td><td>${money(row.estimated_total)}</td><td><span class="status-chip">${esc(row.status)}</span></td></tr>`,"No queued stock orders")}`; }
...
async function renderLivePositions() {
  ...
  const equityCount=rows.filter(r=>r.position_type==="equity").length, leverageCount=rows.filter(r=>r.position_type==="leverage").length, shortCount=rows.filter(r=>r.side==="short").length, profitable=rows.filter(r=>Number(r.unrealized_pnl)>=0).length;
  return `${sectionHead("LIVE POSITIONS", "Resident market exposure", "Monitor every open stock holding and leveraged long/short position with matching account, community, entry cost, current value, unrealized P/L, and liquidation context.")}${metrics([["Open positions",rows.length,"Current filtered result"],["Stock holdings",equityCount,"Open share positions"],["Leverage",leverageCount,`${shortCount} short`],["Profitable",profitable,"Unrealized gain"],["Losing",rows.length-profitable,"Unrealized loss"],["Current value",money(rows.reduce((s,r)=>s+Number(r.current_value||0),0)),"Marked exposure"]])}<form id="position-filter-form" class="panel form-grid"><label>Community<input name="community" value="${fieldValue(f.community||"")}" placeholder="CAD community"></label><label>Stock symbol<input name="ticker" value="${fieldValue(f.ticker||"")}" placeholder="FCX ticker"></label><label>Account or resident<input name="account" value="${fieldValue(f.account||"")}" placeholder="Name or account ID"></label><label>Position type<select name="position_type"><option value="">Stocks and leverage</option><option value="equity" ${f.position_type==="equity"?"selected":""}>Stock holdings only</option><option value="leverage" ${f.position_type==="leverage"?"selected":""}>Leverage only</option></select></label><label>Direction<select name="side"><option value="">Long and short</option><option value="long" ${f.side==="long"?"selected":""}>Long only</option><option value="short" ${f.side==="short"?"selected":""}>Short only</option></select></label><label>Performance<select name="outcome"><option value="">Profitable and losing</option><option value="profitable" ${f.outcome==="profitable"?"selected":""}>Profitable</option><option value="losing" ${f.outcome==="losing"?"selected":""}>Losing</option></select></label><div class="button-row"><button class="primary">Apply filters</button><button type="button" id="clear-position-filters">Clear</button></div></form>${positionsTable(rows)}`;
}
...
async function renderSeizedAssets() {
  const data=await request("/admin/fec/workspace"),accounts=data.accounts||[],ledger=data.asset_ledger||[],pool=data.asset_pool||{},readOnly=currentRoles().has("fec_investigator")&&!isDeveloper(),accountOptions=accounts.map(row=>`<option value="${row.market_account_id}">${esc(row.display_name||row.account_id)} — ${esc(row.account_id)} · ${money(row.cash_balance)}</option>`).join("");
  const controls=readOnly?`<article class="panel wide"><strong>Read-only custody access</strong><p>FEC Investigators may review every seizure and disposition. An FEC Administrator, Commissioner, or Developer must execute custody actions.</p></article>`:`<div class="grid operator-grid"><article class="panel"><h4>Return, delete, or reinvest held funds</h4><form id="fec-disposition-form" class="form-grid"><label>Action<select name="disposition"><option value="return">Return to the investigated user</option><option value="forfeit">Delete from the system permanently</option><option value="reinvest">Reinvest across the market by market cap</option></select></label><label>Amount<input type="number" name="amount" min="0.01" max="${Math.min(Number(pool.balance||0),50000000000)}" step="0.01" required><small>Up to the current custody balance; maximum $50 billion per audited disposition.</small></label><label class="full">Return account<select name="target_account_id"><option value="">Not needed for delete/reinvest</option>${accountOptions}</select></label><label>Case reference<input name="case_reference" minlength="3" required></label><label class="full">Disposition order / rationale<textarea name="reason" minlength="10" required></textarea></label><label>Typed authorization<input name="authorization" placeholder="FORECLOSE" autocomplete="off" required><small>Type FORECLOSE (capitalization is accepted automatically).</small></label><button class="primary full">Execute seized-asset disposition</button></form></article><article class="panel"><h4>Disposition behavior</h4><dl class="state-list"><div><dt>Return</dt><dd>Credits the selected Ravenhood cash account</dd></div><div><dt>Delete</dt><dd>Removes the amount from circulation</dd></div><div><dt>Reinvest</dt><dd>Allocates across active securities in proportion to current market capitalization</dd></div><div><dt>Audit</dt><dd>Every action retains case, reason, officer, and allocation evidence</dd></div></dl></article></div>`;
  return `${sectionHead("FEC ASSET CUSTODY","Seized Assets","Funds removed during an investigation remain in the FEC custody pool until an authorized disposition returns, deletes, or reinvests them.")}${metrics([["Held by FEC",money(pool.balance),`Updated ${stamp(pool.updated_at)}`],["Custody events",number(ledger.length),"Permanent ledger"],["Seizures",number(ledger.filter(row=>row.event_type==="seizure").length),"Funds received"],["Returns",number(ledger.filter(row=>row.event_type==="return").length),"Resident credits"],["Reinvestments",number(ledger.filter(row=>row.event_type==="reinvest").length),"Market-cap allocations"],["Deleted",money(ledger.filter(row=>row.event_type==="forfeit").reduce((sum,row)=>sum+Number(row.amount||0),0)),"Removed from circulation"]])}${controls}${sectionHead("CUSTODY LEDGER","All seized-asset activity","Follow every seizure and final disposition back to its investigation case and authorizing officer.")}${dataTable(["Time","Action","Amount","Pool after","Resident / destination","Case","Reason","Officer"],ledger,row=>`<tr><td>${esc(stamp(row.created_at))}</td><td><strong>${esc(row.event_type==="forfeit"?"deleted":row.event_type)}</strong></td><td>${money(row.amount)}</td><td>${money(row.pool_balance_after)}</td><td>${esc(row.target_name||row.target_account_id||(row.event_type==="reinvest"?"Market-wide":"FEC custody"))}</td><td>${esc(row.case_reference)}</td><td>${esc(row.reason)}</td><td>${esc(row.created_by_name)}</td></tr>`)}`;
}

function activeInvestigationRestrictions() {
  const rows=state.investigationRestrictions||[],readOnly=currentRoles().has("fec_investigator")&&!isDeveloper();
  return `<section class="investigation-restrictions"><div class="section-head"><div><p class="eyebrow">ENFORCEMENT STATUS</p><h3>Active trading restrictions</h3><p>Every resident restriction currently blocking stock trading, leverage trading, or the complete Ravenhood account.</p></div><span class="status-chip ${rows.length?"danger":""}">${number(rows.length)} ACTIVE</span></div>${dataTable(["Resident / identity","Scope","Reason","Case","Issued by","Issued",...(readOnly?[]:["Action"])],rows,row=>`<tr><td>${linkedIdentity(row)}</td><td><span class="status-chip danger">${esc(String(row.scope||"full").toUpperCase())}</span></td><td>${esc(row.reason)}</td><td>${esc(row.case_reference||"—")}</td><td>${esc(row.created_by_name||"FEC automated surveillance")}</td><td>${esc(stamp(row.created_at))}</td>${readOnly?"":`<td><button class="release-restriction" data-id="${row.id}" data-account="${esc(row.display_name||row.ravenhood_account_id)}">Review & release</button></td>`}</tr>`,"No active resident trading restrictions")}</section>`;
}

async function renderInvestigationsEnhanced() {
  const [accounts,surveillance,workspace]=await Promise.all([cached("accounts","/admin/accounts?limit=500"),request("/admin/investigations/profit-surveillance"),request("/admin/fec/workspace")]),investigator=currentRoles().has("fec_investigator"),s=surveillance.settings||{},flags=surveillance.flags||[];
  state.investigationAccounts=accounts;
  state.investigationRestrictions=(workspace.restrictions||[]).filter(row=>row.status==="active");
  const policy=`${sectionHead("AUTOMATED SURVEILLANCE", "Excessive-profit trading flag", "Automatically flag and restrict Ravenhood accounts whose combined trading gains cross the configured amount inside the selected time window.")}<form id="profit-surveillance-form" class="panel form-grid"><label>Rule status<select name="enabled" ${investigator?"disabled":""}><option value="true" ${s.enabled?"selected":""}>Enabled</option><option value="false" ${!s.enabled?"selected":""}>Disabled</option></select></label><label>Gain threshold<input name="gain_threshold" type="number" min="1" step="1" value="${fieldValue(s.gain_threshold||1000000000)}" ${investigator?"disabled":""}></label><label>Time window (hours)<input name="window_hours" type="number" min="1" max="8760" value="${fieldValue(s.window_hours||24)}" ${investigator?"disabled":""}></label><label>Include open/unrealized P/L<select name="include_unrealized" ${investigator?"disabled":""}><option value="true" ${s.include_unrealized?"selected":""}>Yes</option><option value="false" ${!s.include_unrealized?"selected":""}>No</option></select></label><label>Automatically restrict trading<select name="auto_restrict" ${investigator?"disabled":""}><option value="true" ${s.auto_restrict?"selected":""}>Yes — shares and leverage</option><option value="false" ${!s.auto_restrict?"selected":""}>Flag only</option></select></label>${investigator?"":`<div class="button-row full"><button class="primary" type="submit">Save surveillance rule</button><button type="button" id="profit-surveillance-scan">Scan all accounts now</button></div>`}</form>${dataTable(["Case","Account","Priority","Status","Evidence","Updated","Action"],flags,r=>`<tr><td><strong>${esc(r.case_id)}</strong></td><td>${esc(r.display_name||r.account_id)}</td><td>${esc(r.priority)}</td><td>${esc(r.status)}</td><td>${esc(r.summary)}</td><td>${esc(stamp(r.updated_at))}</td><td><button class="open-flag-investigation" data-account="${esc(r.account_id)}" data-case="${esc(r.case_id)}">Investigate</button></td></tr>`)}`;
  return `${sectionHead("FEC INVESTIGATIONS", "Complete resident financial history", "Select a Ravenhood account to open its authoritative identity, active leverage, period P/L analytics, and server-paginated transaction history.")}${policy}<form id="investigation-account-form" class="panel form-grid"><label class="full">Resident / Ravenhood account<select name="account_id" required><option value="">Choose an account to investigate</option>${(accounts.accounts||[]).map(row=>`<option value="${esc(row.account_id)}" ${state.investigation.accountId===row.account_id?"selected":""}>${esc(row.display_name||row.account_id)} — ${esc(row.account_id)} · ${esc(row.communities||"Unlinked")}</option>`).join("")}</select></label><label>Analysis period<select name="period"><option value="day" ${state.investigation.period==="day"?"selected":""}>DAY</option><option value="week" ${state.investigation.period==="week"?"selected":""}>WEEK</option><option value="month" ${state.investigation.period==="month"?"selected":""}>MONTH</option><option value="year" ${state.investigation.period==="year"?"selected":""}>YEAR</option></select></label><label>History search<input name="search" value="${fieldValue(state.investigation.search)}" placeholder="Symbol or transaction type"></label><button class="primary full">Open financial profile</button></form><div id="investigation-account-result">${state.investigation.accountId?`<div class="empty">Loading account investigation…</div>`:`<div class="empty">Select a resident account to begin an investigation.</div>`}</div>${investigator?"":`<details class="legacy-tools"><summary>Advanced FEC casework, restrictions, custody, issuers, and reset tools</summary><div id="advanced-investigation-tools" class="legacy-tools-content"><button id="load-advanced-investigations">Load advanced regulatory workspace</button></div></details>`}`;
}

async function renderProfessionalAccounts() {
  const data=await cached("accounts","/admin/accounts?limit=500"),accounts=[...(data.accounts||[])].sort((a,b)=>String(a.display_name||a.account_id).localeCompare(String(b.display_name||b.account_id),undefined,{sensitivity:"base"}));
  const communities=[...new Set(accounts.flatMap(row=>String(row.communities||"").split(",").map(value=>value.trim()).filter(Boolean)))].sort((a,b)=>a.localeCompare(b,undefined,{sensitivity:"base"}));
  return `${sectionHead("RAVENHOOD ACCOUNTS", "Resident account directory", "Search, filter, and sort compact account records. Expand only the resident profile you need; trading statistics, activity, holdings, and leverage load on demand.")}${metrics([["Accounts",accounts.length,"Central identities"],["Linked communities",accounts.reduce((s,r)=>s+Number(r.community_count||0),0),"Verified relationships"],["Brokerage cash",money(accounts.reduce((s,r)=>s+Number(r.cash_balance||0),0)),"FCX ledger"],["Holdings",money(accounts.reduce((s,r)=>s+Number(r.holdings_value||0),0)),"Marked value"]])}<div class="panel wide"><div class="account-directory-head"><div><h4>Account directory</h4><p>Alphabetical by default. Narrow the directory by identity, community, status, or portfolio value.</p></div><strong id="account-directory-count">${accounts.length} shown</strong></div><div class="account-directory-controls"><label><span>Search</span><input id="account-directory-search" type="search" placeholder="Resident, Ravenhood ID, or Bohemia identity" aria-label="Search Ravenhood accounts"></label><label><span>Community</span><select id="account-directory-community"><option value="">All communities</option><option value="unlinked">Unlinked</option>${communities.map(value=>`<option value="${esc(value.toLowerCase())}">${esc(value)}</option>`).join("")}</select></label><label><span>Status</span><select id="account-directory-status"><option value="">All statuses</option><option value="active">Active</option><option value="restricted">Restricted</option><option value="suspended">Suspended</option><option value="closed">Closed</option></select></label><label><span>Sort</span><select id="account-directory-sort"><option value="name-asc">Resident A–Z</option><option value="name-desc">Resident Z–A</option><option value="value-desc">Portfolio: highest first</option><option value="value-asc">Portfolio: lowest first</option><option value="community-asc">Community A–Z</option><option value="status-asc">Status A–Z</option></select></label></div><div class="account-stack" id="account-directory-list">${accounts.map(r=>{const value=Number(r.cash_balance||0)+Number(r.holdings_value||0),status=String(r.trading_status||r.status||"unknown").toLowerCase(),community=String(r.communities||"").toLowerCase();return `<details class="panel account-card" data-account="${esc(r.account_id)}" data-search="${esc(`${r.display_name} ${r.account_id} ${r.bohemia_identity_id} ${r.communities}`.toLowerCase())}" data-name="${esc(String(r.display_name||r.account_id).toLowerCase())}" data-community="${esc(community||"unlinked")}" data-status="${esc(status)}" data-value="${value}"><summary><span><strong>${esc(r.display_name||r.account_id)}</strong><small>${esc(r.account_id)} · ${esc(r.communities||"Unlinked")} · ${esc(r.trading_status||r.status)}</small></span><span>${money(value)} portfolio value</span></summary><div class="account-detail empty">Open to load the complete resident profile…</div></details>`;}).join("")||`<div class="empty">No Ravenhood accounts</div>`}</div><div class="empty compact" id="account-directory-empty" hidden>No accounts match these filters.</div></div>`;
}
...
function bindActions() {
  ...
  document.querySelectorAll(".risk-alert-review").forEach(button=>button.addEventListener("click",async()=>{const target=$("#alert-review-target");if(!target)return;target.innerHTML=`<div class="empty">Loading alert evidence and advised adjustment…</div>`;try{const data=await request(`/admin/risk-flags/${button.dataset.id}`),flag=data.flag||{},recommendation=data.recommendation||{},recommended=Object.entries(recommendation.settings||{}),current=recommendation.current||{},canAdjust=isDeveloper()||currentRoles().has("fec_admin");target.innerHTML=`${sectionHead("ALERT REVIEW",`${flag.ticker||flag.security_id} · ${String(flag.flag_type||"risk signal").replaceAll("_"," ")}`,"Review the observed evidence against the current engine threshold. Recommended values include a measured buffer above this event and remain editable before applying.")}<article class="panel wide"><div class="panel-heading"><div><h4>${esc(flag.name||flag.ticker||"Market signal")}</h4><p>${esc(flag.severity)} · first seen ${esc(stamp(flag.first_seen_at))} · last seen ${esc(stamp(flag.last_seen_at))}</p></div><span class="status-chip warn">${esc(flag.status)}</span></div><div class="notice"><strong>Advised adjustment</strong><p>${esc(recommendation.explanation||"Review manually.")}</p></div><h4>Evidence</h4><code>${esc(safeJson(flag.evidence_json))}</code>${recommended.length?`<form id="risk-alert-adjustment-form" class="form-grid"><input type="hidden" name="flag_id" value="${flag.id}">${recommended.map(([key,value])=>`<label class="full">${esc(key.replaceAll("_"," "))}<small>Current: ${number(current[key])}</small><input name="${esc(key)}" type="number" step="0.01" value="${fieldValue(value)}" ${canAdjust?"":"disabled"}></label>`).join("")}<label class="full">Review note<textarea name="note" minlength="3" required ${canAdjust?"":"disabled"}>Applied the advised threshold adjustment after reviewing this alert's evidence.</textarea></label>${canAdjust?`<div class="button-row full"><button class="primary" type="submit">Apply advised adjustment and resolve</button><button type="button" id="dismiss-risk-alert">Dismiss without adjustment</button></div>`:"<p class=\"muted full\">Read-only investigators can review evidence; an FEC Administrator must apply or dismiss adjustments.</p>"}</form>`:`<p class="muted">This alert has no automated threshold recommendation. Review it in FCX Engine.</p>`}</article>`;$("#risk-alert-adjustment-form")?.addEventListener("submit",async event=>{event.preventDefault();const raw=formBody(event.currentTarget),payload={};recommended.forEach(([key])=>payload[key]=Number(raw[key]));await request("/admin/engine/settings",{method:"PATCH",body:payload});await request(`/admin/risk-flags/${flag.id}/disposition`,{method:"POST",body:{status:"resolved",note:raw.note}});state.cache.clear();notice("Advised engine threshold applied and alert resolved.");await openSection("overview",true);});$("#dismiss-risk-alert")?.addEventListener("click",async()=>{const note=prompt("Reason for dismissing this alert without changing thresholds:");if(!note||note.trim().length<3)return;await request(`/admin/risk-flags/${flag.id}/disposition`,{method:"POST",body:{status:"dismissed",note}});state.cache.clear();notice("Risk alert dismissed with an audit note.");await openSection("overview",true);});target.scrollIntoView({behavior:"smooth",block:"center"});}catch(error){target.innerHTML=`<div class="empty">${esc(error.message)}</div>`;}}));
  ...
  document.querySelectorAll(".position-price-change").forEach(button=>button.addEventListener("click",async()=>{
    const target=Number(button.dataset.entry);
    if(!Number.isFinite(target)||target<=0)return notice("This position does not have a valid entry price.",true);
    if(!confirm(`Set ${button.dataset.ticker} to ${money(target)} so this position returns to $0 P/L? This updates the shared market quote.`))return;
    button.disabled=true;
    await mutation(`/admin/live-positions/${button.dataset.id}/price`,"POST",{target_price:target,reason:"FEC one-click break-even price intervention"},`${button.dataset.ticker} set to ${money(target)}; selected position P/L reset to break-even.`,state.section);
  }));
  document.querySelectorAll(".position-liquidate").forEach(button=>button.addEventListener("click",async()=>{
    if(!confirm(`LIQUIDATE LEVERAGED POSITION?\n\n${button.dataset.account} · ${button.dataset.ticker}\n\nThe position will close at the current quote, remaining payout will settle to the resident account, and the action will be audit logged.`))return;
    button.disabled=true;
    await mutation(`/admin/live-positions/${button.dataset.id}/liquidate`,"POST",{reason:"FEC operator liquidation from live position overview"},`${button.dataset.ticker} leveraged position liquidated.`,state.section);
  }));
  const applyAccountDirectory=()=>{const query=$("#account-directory-search")?.value.trim().toLowerCase()||"",community=$("#account-directory-community")?.value||"",status=$("#account-directory-status")?.value||"",sort=$("#account-directory-sort")?.value||"name-asc",list=$("#account-directory-list"),cards=Array.from(list?.querySelectorAll("details.account-card")||[]);cards.sort((a,b)=>{if(sort==="name-desc")return b.dataset.name.localeCompare(a.dataset.name);if(sort==="value-desc")return Number(b.dataset.value)-Number(a.dataset.value);if(sort==="value-asc")return Number(a.dataset.value)-Number(b.dataset.value);if(sort==="community-asc")return a.dataset.community.localeCompare(b.dataset.community)||a.dataset.name.localeCompare(b.dataset.name);if(sort==="status-asc")return a.dataset.status.localeCompare(b.dataset.status)||a.dataset.name.localeCompare(b.dataset.name);return a.dataset.name.localeCompare(b.dataset.name);}).forEach(card=>list.appendChild(card));let shown=0;cards.forEach(card=>{const communityMatch=!community||(community==="unlinked"?card.dataset.community==="unlinked":card.dataset.community.split(",").map(value=>value.trim()).includes(community)),statusMatch=!status||card.dataset.status===status,queryMatch=!query||card.dataset.search.includes(query);card.hidden=!(communityMatch&&statusMatch&&queryMatch);if(!card.hidden)shown++;});if($("#account-directory-count"))$("#account-directory-count").textContent=`${shown} of ${cards.length} shown`;if($("#account-directory-empty"))$("#account-directory-empty").hidden=shown!==0;};
  ["#account-directory-search","#account-directory-community","#account-directory-status","#account-directory-sort"].forEach(selector=>$(selector)?.addEventListener(selector.includes("search")?"input":"change",applyAccountDirectory));applyAccountDirectory();
  document.querySelectorAll("details.account-card").forEach(card=>card.addEventListener("toggle",async()=>{if(!card.open||card.dataset.loaded)return;const target=card.querySelector(".account-detail");try{const d=await request(`/admin/accounts/${encodeURIComponent(card.dataset.account)}/profile`),a=d.account,s=d.stats||{},open=residentOpenPositions(d,a);target.classList.remove("empty");target.innerHTML=`${metrics([["Portfolio",money(Number(a.cash_balance||0)+(d.holdings||[]).reduce((x,h)=>x+Number(h.current_value||0),0)),"Cash + holdings"],["Cash",money(a.cash_balance),a.trading_status],["Buys",number(s.total_buys),money(s.buy_volume)],["Sells",number(s.total_sells),money(s.sell_volume)],["Realized P/L",pnl(s.realized_pnl),`${number(s.closed_positions)} closed`,"",true],["Unrealized P/L",pnl(open.reduce((x,p)=>x+Number(p.unrealized_pnl||0),0)),`${number(open.length)} open positions`,"",true]])}${pendingOrdersTable(d.pending_orders||[])}<h4>OPEN POSITIONS — STOCKS & LEVERAGE</h4>${positionsTable(open)}${dataTable(["Time","Symbol","Type","Quantity","Price","Value"],d.recent_activity||[],r=>`<tr><td>${esc(stamp(r.created_at))}</td><td>${esc(r.ticker)}</td><td>${esc(r.side)}</td><td>${number(r.quantity)}</td><td>${money(r.unit_price)}</td><td>${money(r.gross_amount)}</td></tr>`)}`;card.dataset.loaded="1";}catch(e){target.textContent=e.message;}}));
  ...
  $("#profit-surveillance-form")?.addEventListener("submit",async event=>{event.preventDefault();const raw=formBody(event.currentTarget);await mutation("/admin/investigations/profit-surveillance","PATCH",{enabled:raw.enabled==="true",gain_threshold:Number(raw.gain_threshold),window_hours:Number(raw.window_hours),include_unrealized:raw.include_unrealized==="true",auto_restrict:raw.auto_restrict==="true"},"Profit surveillance rule saved.","investigations");});
  $("#profit-surveillance-scan")?.addEventListener("click",async()=>{if(!confirm("Scan every Ravenhood account now and apply the configured automatic restrictions?"))return;await mutation("/admin/investigations/profit-surveillance/scan","POST",{},"Profit surveillance scan completed.","investigations");});
  const investigator=currentRoles().has("fec_investigator"),accounts=state.investigationAccounts||{accounts:[]};
  const appendInvestigationActions=()=>{if(investigator)return;const selected=(accounts.accounts||[]).find(row=>row.account_id===state.investigation.accountId),target=$("#investigation-account-result");if(!selected||!target)return;const caseReference=state.investigation.caseReference||"";target.insertAdjacentHTML("beforeend",`${sectionHead("CASE ACTIONS","Investigation authority","Create a retained case and place settled resident cash into FEC custody without leaving the resident's trade and leverage evidence.")}<div class="grid operator-grid"><form id="investigation-case-form" class="panel form-grid"><h4 class="full">Create investigation</h4><label>Priority<select name="priority"><option value="normal">Normal</option><option value="high">High</option><option value="critical">Critical</option><option value="low">Low</option></select></label><label class="full">Summary<textarea name="summary" minlength="10" required></textarea></label><button class="primary full">Open retained FEC case</button></form><form id="investigation-seizure-form" class="panel form-grid"><h4 class="full">Seize settled cash</h4><label>Available cash<input value="${money(selected.cash_balance)}" disabled></label><label>Amount<input name="amount" type="number" min="0.01" max="${Number(selected.cash_balance||0)}" step="0.01" required></label><label>Case reference<input name="case_reference" value="${fieldValue(caseReference)}" required></label><label class="full">Legal basis / evidence<textarea name="reason" minlength="10" required></textarea></label><label>Typed authorization<input name="authorization" placeholder="SEIZE" required></label><button class="danger-action full">Move funds to FEC custody</button></form></div><p class="muted">Final custody actions are managed from the Seized Assets tab.</p>`);$("#investigation-case-form")?.addEventListener("submit",async event=>{event.preventDefault();const raw=formBody(event.currentTarget),created=await request("/admin/investigations",{method:"POST",body:{account_id:selected.account_id,priority:raw.priority,summary:raw.summary}});state.investigation.caseReference=created.case_id;state.cache.clear();notice(`Investigation ${created.case_id} opened.`);await runInvestigation(state.investigation.page||1);appendInvestigationActions();});$("#investigation-seizure-form")?.addEventListener("submit",async event=>{event.preventDefault();const raw=formBody(event.currentTarget);await mutation("/admin/fec/custody/seize","POST",{account_id:Number(selected.market_account_id),amount:Number(raw.amount),case_reference:raw.case_reference,reason:raw.reason,authorization:raw.authorization},"Resident funds moved into FEC custody.","seized");});};
  const runInvestigation=async(page=1)=>{const form=$("#investigation-account-form"),target=$("#investigation-account-result");if(!form||!target)return;const raw=formBody(form);if(!raw.account_id)return;state.investigation={accountId:raw.account_id,period:raw.period||"day",search:raw.search||"",page};target.innerHTML=`<div class="empty">Loading authoritative account history…</div>`;try{const id=encodeURIComponent(raw.account_id),query=new URLSearchParams({page:String(page),page_size:"50",search:raw.search||"",sort:"desc"});const [h,a]=await Promise.all([request(`/admin/investigations/accounts/${id}/history?${query}`),request(`/admin/investigations/accounts/${id}/analytics?period=${raw.period||"day"}`)]),x=a.analytics||{},p=h.pagination||{};target.innerHTML=`<article class="panel wide"><div class="panel-heading"><div><p class="eyebrow">ACCOUNT IDENTITY</p><h4>${esc(h.account.display_name||h.account.account_id)}</h4><p>${esc(h.account.account_id)} · ${esc(h.account.communities||"Unlinked")} · ${esc(h.account.status)}</p></div><span class="status-chip">${esc(h.account.status||"Active")}</span></div>${metrics([["Cash balance",money(h.account.cash_balance),"FCX ledger"],["Realized P/L",pnl(x.realized_pnl),`Equity ${pnl(x.equity_realized_pnl)} · leverage ${pnl(x.leverage_realized_pnl)}`,"",true],["Unrealized P/L",pnl(x.unrealized_pnl),"Shares plus leverage","",true],["Combined P/L",pnl(x.combined_pnl),"Period result","",true],["Buy volume",money(x.buy_volume),`${number(x.buys)} buys`],["Sell volume",money(x.sell_volume),`${number(x.sells)} sells`],["Trades",number(x.trades),`${number(x.winning_trades)} wins / ${number(x.losing_trades)} losses`]])}</article>${sectionHead("ACTIVE LEVERAGE","Current account exposure","Open leveraged positions are shown immediately beneath the linked Ravenhood identity.")}${positionsTable((h.active_leverage||[]).map(position=>({...position,display_name:h.account.display_name,account_id:h.account.account_id,communities:h.account.communities})))}${sectionHead("TRANSACTION HISTORY","Complete available trading ledger",`${number(p.total)} records · server-filtered and paginated`)}${dataTable(["Timestamp","Symbol","Transaction","Direction","Quantity","Execution price","Value","Cost basis","Leverage","Trade P/L"],h.history||[],r=>`<tr><td>${esc(stamp(r.occurred_at))}</td><td><strong>${esc(r.ticker)}</strong></td><td>${esc(r.transaction_type)}</td><td>${esc(r.direction)}</td><td>${number(r.quantity)}</td><td>${money(r.price)}</td><td>${money(r.value)}</td><td>${r.cost_basis==null?"—":money(r.cost_basis)}</td><td>${r.leverage?`${number(r.leverage)}x`:"—"}</td><td>${r.realized_pnl==null?"—":pnl(r.realized_pnl)}</td></tr>`)}<div class="button-row"><button id="investigation-prev" ${page<=1?"disabled":""}>Previous</button><span class="muted">Page ${number(p.page)} of ${number(p.pages)}</span><button id="investigation-next" ${page>=Number(p.pages||1)?"disabled":""}>Next</button></div>`;$("#investigation-prev")?.addEventListener("click",()=>runInvestigation(page-1));$("#investigation-next")?.addEventListener("click",()=>runInvestigation(page+1));}catch(error){target.innerHTML=`<div class="empty">${esc(error.message)}</div>`;}};
  $("#investigation-account-form")?.addEventListener("submit",async event=>{event.preventDefault();event.stopImmediatePropagation();await runInvestigation(1);appendInvestigationActions();});
  document.querySelectorAll(".open-flag-investigation").forEach(button=>button.addEventListener("click",async()=>{const caseReference=button.dataset.case;state.investigation.accountId=button.dataset.account;const form=$("#investigation-account-form");if(form){form.elements.account_id.value=button.dataset.account;form.elements.period.value=state.investigation.period||"day";}await runInvestigation(1);state.investigation.caseReference=caseReference;appendInvestigationActions();$("#investigation-account-result")?.scrollIntoView({behavior:"smooth",block:"start"});}));
  ...
  const filterSecurityDirectory=()=>{const query=$("#security-directory-search")?.value.trim().toLowerCase()||"",status=$("#security-directory-status")?.value||"",rows=Array.from(document.querySelectorAll("[data-security-directory-row]"));let shown=0;rows.forEach(row=>{row.hidden=Boolean((status&&row.dataset.status!==status)||(query&&!row.dataset.search.includes(query)));if(!row.hidden)shown++;});if($("#security-directory-count"))$("#security-directory-count").textContent=`${shown} of ${rows.length} shown`;if($("#security-directory-empty"))$("#security-directory-empty").hidden=shown!==0;};
  $("#security-directory-search")?.addEventListener("input",filterSecurityDirectory);$("#security-directory-status")?.addEventListener("change",filterSecurityDirectory);filterSecurityDirectory();
  ...
  document.querySelectorAll(".release-restriction").forEach(button => button.addEventListener("click", async () => { const note=prompt(`Release restriction for ${button.dataset.account||"this resident"}? Enter the review finding:`); if(!note?.trim())return; await mutation(`/admin/fec/restrictions/${button.dataset.id}/release`, "POST", { note: note.trim() }, "Account trading restriction released.", "investigations"); }));
  $("#fec-seizure-form")?.addEventListener("submit", async event => {
    event.preventDefault(); const raw = formBody(event.currentTarget);
    await mutation("/admin/fec/custody/seize", "POST", { account_id: Number(raw.account_id), amount: Number(raw.amount), case_reference: raw.case_reference, reason: raw.reason, authorization: raw.authorization }, "Resident FCX cash placed in FEC custody.", state.section);
  });
