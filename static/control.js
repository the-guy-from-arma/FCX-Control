...
const sections = [
  ...
  ["operations", "Companies & Securities"], ["accounts", "Ravenhood Accounts"],
  ["promotions", "Account Promotions"], ["banking", "FCX Banking"], ["investigations", "FEC Investigations"],
  ["leverage", "Leverage Settings"], ["health", "System Health"], ["onboarding", "FEC Onboarding"],
  ["audit", "Audit / Activity"], ["communities", "Community Registry"], ["connections", "API Connections"],
];
...
async function renderCommandOverview() {
  ...
  const marketOpen = yes(settings.market_open), trading = marketOpen, buying = yes(settings.buy_enabled ?? "1"), selling = yes(settings.sell_enabled ?? "1");
  const emergency = admin ? `<div class="command-deck emergency-deck"><button class="emergency-command danger-action" data-command="pause">Halt engine</button><button class="emergency-command" data-command="resume">Resume engine</button><button class="market-state danger-action" data-field="manual_override" data-value="closed">Halt market / trading</button><button class="market-state" data-field="manual_override" data-value="open">Resume market / trading</button>${[["buy_enabled",buying,"Buying"],["sell_enabled",selling,"Selling"]].map(([f,on,l])=>`<button class="market-state ${on?"":"danger-action"}" data-field="${f}" data-value="${on?"false":"true"}">${on?"Disable":"Enable"} ${l}</button>`).join("")}</div>` : `<div class="control-note">Emergency controls require the FEC Administrator role.</div>`;
  return `${sectionHead("FCX COMMAND CENTER", "Overview", "Live regulatory, market, community, risk, and emergency state from the authoritative FCX services.")}
    ${metrics([["Market",marketOpen?"OPEN":"HALTED","Authoritative session",marketOpen?"up":"down"],["FCX engine",yes(engineSettings.enabled)?"RUNNING":"HALTED",engine.state?.status||"Current state",yes(engineSettings.enabled)?"up":"down"],["Live positions",positions.positions?.length||0,"Latest open exposure"],["Active leverage",(data.live_positions||[]).length,money((data.live_positions||[]).reduce((sum,row)=>sum+Number(row.entry_notional||0),0))],["Active companies",data.totals?.securities||0,"Operational listings"],["Trading",trading?"ENABLED":"DISABLED",`Buy ${buying?"on":"off"} / Sell ${selling?"on":"off"}`,trading?"up":"down"]])}
    ${sectionHead("EMERGENCY CONTROLS", "Engine and market authority", "Halts require confirmation and every transition is server-authorized and audit logged.")}${emergency}
    <div class="grid operator-grid"><article class="panel wide"><h4>Live Positions Monitor</h4>${positionsTable(positions.positions||[])}</article>
    <article class="panel"><h4>Quick market movement</h4><p>Launch an audited movement without leaving Overview.</p><form id="quick-movement-form" class="form-grid"><label>Stock<select name="security_ids" required>${selectOptions(operations.securities||[])}</select></label><label>Movement %<input name="percent_change" type="number" min="-99.99" max="1000" step="0.01" required></label><label>Duration minutes<input name="duration_minutes" type="number" min="1" max="43200" value="60" required></label><label class="full">Reason<input name="event_name" minlength="3" required placeholder="FEC-authorized market event"></label><button class="primary full" ${admin?"":"disabled"}>Review and execute</button></form></article></div>
    ${sectionHead("MARKET SETTINGS", "Current market configuration", "These are the same FCX settings used by the exchange; no duplicate configuration is created.")}
    <div class="grid"><article class="panel"><dl class="state-list">${[["Session",settings.market_manual_override||"schedule"],["Trading",trading?"On":"Off"],["Buying",buying?"On":"Off"],["Selling",selling?"On":"Off"],["Trade fee",`${settings.market_trade_fee_percent||0}%`],["Transfer fee",`${settings.market_transfer_fee_percent||0}%`]].map(([k,v])=>`<div><dt>${k}</dt><dd>${esc(v)}</dd></div>`).join("")}</dl></article><article class="panel"><h4>Community status</h4>${communitySummary(data.communities||[])}</article><article class="panel"><h4>Recent FEC actions</h4>${auditSummary(data.recent_actions||[])}</article><article class="panel"><h4>Recent alerts</h4>${auditSummary(data.recent_alerts||[])}</article></div>`;
}

function positionsTable(rows) { return dataTable(["Account / community","Symbol","Position","Quantity","Entry / current","Value","P/L","Leverage","Liquidation","Age"], rows, row=>`<tr><td><strong>${esc(row.display_name||row.account_id)}</strong><small>${esc(row.communities||"Unlinked")}</small></td><td>${esc(row.ticker)}</td><td>${esc(String(row.side||"").toUpperCase())}</td><td>${number(row.quantity)}</td><td>${money(row.entry_price)}<small>${money(row.current_price)}</small></td><td>${money(row.current_value)}</td><td>${pnl(row.unrealized_pnl)}</td><td>${number(row.leverage)}x</td><td>${money(row.liquidation_price)}</td><td>${number(Number(row.age_seconds||0)/3600)}h</td></tr>`, "No active leveraged positions"); }

async function renderLivePositions() {
  const data = await request("/admin/live-positions?limit=1000");
  return `${sectionHead("LIVE POSITIONS", "Resident market exposure", "Filter the authoritative open-position ledger by community, stock, account, direction, leverage, and outcome.")}<form id="position-filter-form" class="panel form-grid"><label>Community<input name="community"></label><label>Stock<input name="ticker"></label><label>Account / resident<input name="account"></label><label>Direction<select name="side"><option value="">All</option><option>long</option><option>short</option></select></label><label>Outcome<select name="outcome"><option value="">All</option><option>profitable</option><option>losing</option></select></label><button class="primary">Filter</button></form>${positionsTable(data.positions||[])}`;
}
...
async function renderOnboarding() {
  ...
  return `${sectionHead("FEC ONBOARDING", "Investigator access management", "Create least-privilege investigators and revoke, restore, or soft-delete access without destroying audit history.")}<div class="grid operator-grid"><form id="investigator-form" class="panel form-grid"><h4>Create Investigator</h4><label class="full">Username<input name="username" minlength="3" required></label><label>Password<input name="password" type="password" minlength="12" required></label><label>Confirm password<input name="confirm_password" type="password" minlength="12" required></label><button class="primary full">Create FEC Investigator</button></form><article class="panel wide">${dataTable(["Username","Role","Status","Created","Last login","Created by","Actions"],data.investigators||[],r=>`<tr><td><strong>${esc(r.display_name)}</strong><small>${esc(r.email)}</small></td><td>FEC Investigator</td><td>${r.deleted_at?"Deleted":yes(r.active)?"Active":"Revoked"}</td><td>${esc(stamp(r.created_at))}</td><td>${esc(stamp(r.last_login_at))}</td><td>${esc(r.created_by_name||"System")}</td><td>${r.deleted_at?"Retained for audit":`<button class="investigator-access" data-id="${r.id}" data-active="${yes(r.active)?0:1}">${yes(r.active)?"Revoke":"Restore"}</button> <button class="investigator-delete danger-action" data-id="${r.id}">Delete</button>`}</td></tr>`)}</article></div>`;
}

async function renderEngineEnhanced() {
  const [html,ops]=await Promise.all([renderEngine(),cached("operations","/admin/operations")]);
  const selector=`${sectionHead("MULTI-STOCK MOVEMENT", "Move selected securities", "Select one, several, or all eligible companies and execute one audited movement program.")}<form id="engine-movement-form" class="panel form-grid"><label class="full">Eligible securities <span id="engine-selected-count">0 selected</span><select name="security_ids" multiple size="12" required>${selectOptions(ops.securities||[])}</select></label><button type="button" id="select-all-engine">Select all eligible</button><label>Movement %<input name="percent_change" type="number" min="-99.99" max="1000" step="0.01" required></label><label>Duration minutes<input name="duration_minutes" type="number" min="1" max="43200" value="60"></label><label class="full">Reason<input name="event_name" minlength="3" required></label><button class="primary full">Confirm movement</button></form>`;
...
async function renderInvestigationsEnhanced() {
  const html=await renderInvestigations();
  return `${sectionHead("ACCOUNT HISTORY", "Complete financial and trading profile", "Search an authoritative Ravenhood account ID. History is filtered and paginated on the server; P/L aggregates are calculated by FCX.")}<form id="investigation-account-form" class="panel form-grid"><label>Ravenhood account ID<input name="account_id" required></label><label>Period<select name="period"><option>day</option><option>week</option><option>month</option><option>year</option></select></label><label>Search<input name="search" placeholder="symbol or transaction type"></label><button class="primary">Open account profile</button></form><div id="investigation-account-result"></div>${currentRoles().has("fec_investigator")?"":html}`;
}

const renderers = { overview: renderCommandOverview, engine: renderEngineEnhanced, positions: renderLivePositions, operations: renderOperations, accounts: renderExpandableAccounts, promotions: renderPromotions, banking: renderBanking, investigations: renderInvestigationsEnhanced, communities: renderCommunities, connections: renderConnections, leverage: renderLeverageEnhanced, onboarding: renderOnboarding, audit: renderAudit, health: renderSystemHealth };
...
function bindActions() {
  ...
  document.querySelectorAll(".market-state").forEach(button=>button.addEventListener("click",async()=>{const value=button.dataset.value;const dangerous=value==="closed"||value==="false";if(dangerous&&!confirm(`CONFIRM MARKET CONTROL\n\nThis will change ${button.textContent.trim().toLowerCase()} immediately for connected communities and will be audit logged.`))return;const parsed=value==="true"?true:value==="false"?false:value;await mutation("/admin/market/settings","PATCH",{[button.dataset.field]:parsed},"Market control updated.","overview");}));
  const movementSubmit=(selector,target)=>$(selector)?.addEventListener("submit",async event=>{event.preventDefault();const form=event.currentTarget,raw=formBody(form);const select=form.elements.security_ids;const security_ids=select.multiple?Array.from(select.selectedOptions).map(o=>Number(o.value)):[Number(select.value)];if(!confirm(`EXECUTE STOCK MOVEMENT?\n\n${security_ids.length} security(s) will move ${raw.percent_change}% over ${raw.duration_minutes} minutes. Final prices remain in effect.`))return;await mutation("/admin/operations/programs","POST",{security_ids,event_name:raw.event_name,percent_change:Number(raw.percent_change),duration_minutes:Number(raw.duration_minutes),starts_at:null},"Audited stock movement initiated.",target);});
  movementSubmit("#quick-movement-form","overview"); movementSubmit("#engine-movement-form","engine");
  const engineSelect=$("#engine-movement-form select[name=security_ids]"); const updateSelected=()=>{if($("#engine-selected-count"))$("#engine-selected-count").textContent=`${engineSelect?.selectedOptions.length||0} selected`;}; engineSelect?.addEventListener("change",updateSelected); $("#select-all-engine")?.addEventListener("click",()=>{Array.from(engineSelect.options).forEach(o=>o.selected=true);updateSelected();}); updateSelected();
  $("#position-filter-form")?.addEventListener("submit",async event=>{event.preventDefault();const q=new URLSearchParams(formBody(event.currentTarget));const data=await request(`/admin/live-positions?${q}`);document.querySelector("#content").innerHTML=`${sectionHead("LIVE POSITIONS","Filtered resident exposure")}${positionsTable(data.positions||[])}`;});
  document.querySelectorAll("details.account-card").forEach(card=>card.addEventListener("toggle",async()=>{if(!card.open||card.dataset.loaded)return;const target=card.querySelector(".account-detail");try{const d=await request(`/admin/accounts/${encodeURIComponent(card.dataset.account)}/profile`),a=d.account,s=d.stats||{};target.classList.remove("empty");target.innerHTML=`${metrics([["Portfolio",money(Number(a.cash_balance||0)+(d.holdings||[]).reduce((x,h)=>x+Number(h.current_value||0),0)),"Cash + holdings"],["Cash",money(a.cash_balance),a.trading_status],["Buys",number(s.total_buys),money(s.buy_volume)],["Sells",number(s.total_sells),money(s.sell_volume)],["Realized P/L",pnl(s.realized_pnl),`${number(s.closed_positions)} closed`],["Unrealized P/L",pnl((d.holdings||[]).reduce((x,h)=>x+Number(h.unrealized_pnl||0),0)+(d.active_leverage||[]).reduce((x,p)=>x+Number(p.unrealized_pnl||0),0)),`${number(s.open_positions)} open`]])}<h4>ACTIVE LEVERAGE</h4>${positionsTable((d.active_leverage||[]).map(p=>({...p,display_name:a.display_name,account_id:a.account_id,communities:a.communities,current_value:Number(p.quantity||0)*Number(p.current_price||0)})))}${dataTable(["Time","Symbol","Type","Quantity","Price","Value"],d.recent_activity||[],r=>`<tr><td>${esc(stamp(r.created_at))}</td><td>${esc(r.ticker)}</td><td>${esc(r.side)}</td><td>${number(r.quantity)}</td><td>${money(r.unit_price)}</td><td>${money(r.gross_amount)}</td></tr>`)}`;card.dataset.loaded="1";}catch(e){target.textContent=e.message;}}));
  $("#investigator-form")?.addEventListener("submit",async event=>{event.preventDefault();const body=formBody(event.currentTarget);await mutation("/admin/fec/investigators","POST",body,"FEC Investigator created.","onboarding");});
  $("#investigation-account-form")?.addEventListener("submit",async event=>{event.preventDefault();const raw=formBody(event.currentTarget),id=encodeURIComponent(raw.account_id),target=$("#investigation-account-result");target.innerHTML=`<div class="empty">Loading complete FCX history…</div>`;try{const [h,a]=await Promise.all([request(`/admin/investigations/accounts/${id}/history?page=1&page_size=50&search=${encodeURIComponent(raw.search||"")}`),request(`/admin/investigations/accounts/${id}/analytics?period=${raw.period}`)]),x=a.analytics||{};target.innerHTML=`${metrics([["Realized P/L",pnl(x.realized_pnl),raw.period],["Unrealized P/L",pnl(x.unrealized_pnl),"Open positions"],["Combined P/L",pnl(x.combined_pnl),"Total"],["Buy volume",money(x.buy_volume),`${number(x.buys)} buys`],["Sell volume",money(x.sell_volume),`${number(x.sells)} sells`],["Trades",number(x.trades),`${number(x.winning_trades)} wins / ${number(x.losing_trades)} losses`]])}<article class="panel"><h4>${esc(h.account.display_name)} · ${esc(h.account.account_id)}</h4><p>${esc(h.account.communities||"Unlinked")} · ${esc(h.account.status)}</p><h4>ACTIVE LEVERAGE</h4>${positionsTable((h.active_leverage||[]).map(p=>({...p,display_name:h.account.display_name,account_id:h.account.account_id,communities:h.account.communities,current_value:Number(p.quantity||0)*Number(p.current_price||0)})))}</article>${dataTable(["Timestamp","Symbol","Transaction","Direction","Quantity","Price","Value","Leverage","Realized P/L"],h.history||[],r=>`<tr><td>${esc(stamp(r.occurred_at))}</td><td>${esc(r.ticker)}</td><td>${esc(r.transaction_type)}</td><td>${esc(r.direction)}</td><td>${number(r.quantity)}</td><td>${money(r.price)}</td><td>${money(r.value)}</td><td>${r.leverage?`${number(r.leverage)}x`:"—"}</td><td>${r.realized_pnl==null?"—":pnl(r.realized_pnl)}</td></tr>`)}<p class="muted">Page ${h.pagination.page} of ${h.pagination.pages} · ${number(h.pagination.total)} records</p>`;}catch(e){target.innerHTML=`<div class="empty">${esc(e.message)}</div>`;}});
  document.querySelectorAll(".investigator-access").forEach(b=>b.addEventListener("click",async()=>{const active=b.dataset.active==="1",reason=prompt(`Reason to ${active?"restore":"revoke"} access:`)||"FEC administrator action";if(!active&&!confirm("Revoke this investigator's authentication and active sessions?"))return;await mutation(`/admin/fec/investigators/${b.dataset.id}`,"PATCH",{active,reason},"Investigator access updated.","onboarding");}));
  document.querySelectorAll(".investigator-delete").forEach(b=>b.addEventListener("click",async()=>{if(!confirm("DELETE INVESTIGATOR ACCOUNT?\n\nLogin access will be permanently removed. Historical audit actions will be retained."))return;await mutation(`/admin/fec/investigators/${b.dataset.id}`,"DELETE",undefined,"Investigator account soft-deleted; audit history retained.","onboarding");}));
