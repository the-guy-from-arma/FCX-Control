const API = "/api/v1";
const state = { user: null, csrf: "", section: "overview", cache: new Map(), settlementFilter: "all", positionFilters: {}, investigation: { page: 1, period: "day", accountId: "", search: "" } };
const sections = [
  ["overview", "Overview"], ["engine", "FCX Engine"], ["positions", "Live Positions"],
  ["operations", "Companies & Securities"], ["leverage", "Leverage Settings"], ["promotions", "Account Promotions"],
...
const sectionMeta = {
  ...
  operations: ["Market Operations", "◆"], leverage: ["Market Operations", "△"], promotions: ["Market Operations", "%"],
...
async function renderCommandOverview() {
  ...
  const emergency = canAdmin ? `<div class="emergency-grid">
    ...
    ${controlTile("FCX Market",marketOpen?"Open":"Halted","All exchange trading",`<button class="market-state ${marketOpen?"danger-action":""}" data-field="manual_override" data-value="${marketOpen?"closed":"open"}">${marketOpen?"Halt":"Resume"}</button>`)}
  ...
  return `<div class="command-header"><div><p class="eyebrow">FCX COMMAND CENTER</p><h3>Operational overview</h3><p>Live market authority, resident exposure, community connectivity, risk signals, and FEC activity from the central FCX control plane.</p></div><div class="command-time"><strong>${esc(new Date().toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"}))}</strong><small>Last synchronized now</small></div></div>
    ${metrics([["Market",marketOpen?"OPEN":"HALTED",settings.market_manual_override||"Scheduled authority",marketOpen?"up":"down"],["FCX Engine",engineRunning?"RUNNING":"HALTED",engine.state?.status||"Autonomous engine",engineRunning?"up":"down"],["CAD Connections",communities.filter(c=>c.connection_enabled&&!c.suspended).length,`${communities.length} registered`],["Live Positions",recentPositions.length,money(exposure)],["Active Companies",data.totals?.securities||0,money(data.totals?.market_cap)],["System Health",serviceHealth,engine.error?"Engine status unavailable":"Core services responding",engine.error?"down":"up"]])}
    <div class="overview-layout"><div class="overview-main">
      <article class="panel emergency-panel"><div class="panel-heading"><div><p class="eyebrow">EMERGENCY AUTHORITY</p><h4>Engine and market controls</h4><p>Every transition requires server authorization and creates an immutable audit entry.</p></div><span class="status-chip ${marketOpen&&engineRunning?"":"danger"}">${marketOpen&&engineRunning?"Normal operations":"Intervention active"}</span></div>${emergency}</article>
      ...
      <article class="panel"><div class="panel-heading"><div><h4>Market configuration</h4><p>Authoritative exchange settings.</p></div></div><dl class="state-list">${[["Session",String(settings.market_manual_override||"schedule").toUpperCase()],["Trading",trading?"Enabled":"Disabled"],["Buying",buying?"Enabled":"Disabled"],["Selling",selling?"Enabled":"Disabled"],["Trade fee",`${settings.market_trade_fee_percent||0}%`],["Transfer fee",`${settings.market_transfer_fee_percent||0}%`]].map(([k,v])=>`<div><dt>${k}</dt><dd>${esc(v)}</dd></div>`).join("")}</dl></article>
